...
export const STORAGE_KEYS = {
  ...
  SHEET_URL: 'salary-calculator-sheet-url',
} as const;
```
...
export const SHIFT_PATTERNS = {
  ...
  // "Thuý" → employee: "Thuý", no shift info
  nameOnly: /^(.+)$/,
 ...
 */
export function parseCell(cellValue: string, date: string): ShiftData {
  ...
  if (withTimeMatch) {
    return {
      ...
      shiftType: withTimeMatch[2] as 'M' | 'N',
      date,
    };
  }

  // Pattern 2: "Thu Na N" (shift only)
  ...
  // Pattern 4: Just name, no shift info
  return {
    employee: trimmed,
    shiftType: null,
    date,
...
export function parseCSV(csvText: string): ParsedSchedule {
  ...
  for (let i = 1; i < rows.length; i++) {
    ...
    for (let j = 1; j < row.length && j <= days.length; j++) {
      const date = days[j - 1];
      rowCells.push(parseCell(row[j] || '', date));
    }
    cells.push(rowCells);
  }
 ...
 */
export function calculateShiftHours(shift: ShiftData): number {
  if (shift.shiftType === null) return 0;
  ...
  if (shift.shiftType === 'M') {
    // Check if there's an implied -14h pattern
    // Default to full M shift (8.5h)
    return SHIFT_HOURS['M'];
  }

  return SHIFT_HOURS[shift.shiftType as keyof typeof SHIFT_HOURS] || 0;
}
...
): Map<string, { totalHours: number; daysWorked: number; shifts: SalaryCalculation['shifts'] }> {
  const employeeData = new Map<
    string,
    { totalHours: number; daysWorked: number; shifts: SalaryCalculation['shifts'] }
  >();
...
export function calculateSalaries(
  ...
  for (const [employeeName, data] of employeeData.entries()) {
    ...
    calculations.push({
      ...
      hourlyWage,
      totalSalary: data.totalHours * hourlyWage,
  ...
  return calculations.sort((a, b) => b.totalSalary - a.totalSalary);
}
```
...
export function setSheetUrl(url: string): void {
  setStorageItem(STORAGE_KEYS.SHEET_URL, url);
}
```
